...
The databricks CLI bundles top level is defined by file `travel_recommendations_model/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`travel_recommendations_model/assets/model-workflow-asset.yml`)
 - batch inference workflow (`travel_recommendations_model/assets/batch-inference-workflow-asset.yml`)
 - monitoring workflow (`travel_recommendations_model/assets/monitoring-workflow-asset.yml`)
 - feature engineering workflow (`travel_recommendations_model/assets/feature-engineering-workflow-asset.yml`)
 - model definition and experiment definition (`travel_recommendations_model/assets/ml-artifacts-asset.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `travel_recommendations_model/databricks.yml` and `travel_recommendations_model/assets/ml-artifacts-asset.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML assets to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-travel_recommendations_model-model     | /dev-travel_recommendations_model-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-travel_recommendations_model-model | /staging-travel_recommendations_model-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-travel_recommendations_model-model    | /prod-travel_recommendations_model-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML assets deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-travel_recommendations_model-model    | /test-travel_recommendations_model-experiment    |
...
2. Complete the `TODO` in `travel_recommendations_model/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `travel_recommendations_model/assets/batch-inference-workflow-asset.yml` to pass
...
To get started, open `travel_recommendations_model/assets/batch-inference-workflow-asset.yml`.  The file contains the ML asset definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-travel_recommendations_model-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-travel_recommendations_model-batch-inference-job`
that runs the notebook under `travel_recommendations_model/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `assets/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-travel_recommendations_model-batch-inference-job`.
...
`test-travel_recommendations_model-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-travel_recommendations_model-batch-inference-job`.
